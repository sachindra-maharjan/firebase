    ...
    .onCreate( async (snapshot, context) =>{
      ...
      if (homeTeam.team_id) {
        ...
        if (teamSnapshot.empty) {
          ...
          updatePlayer(leagueId, teamId, homeTeam);
        }
      }
      ...
      if (awayTeam.team_id) {
        ...
        if (teamSnapshot.empty) {
          ...
          updatePlayer(leagueId, teamId, awayTeam);
        }
      }
      return;
    });
  ...
  try {
    ...
    players.forEach(async function(player:any) {
      ...
      if (!playerData.exists) {
        ...
        playerSnapshot.set(getPlayer(teamId, player, undefined, true))
            .then(() =>{
              functions.logger.info(`New player added in team ${teamId}`);
            }).catch((error) => {
              functions.logger.error(error);
            });
      } else {
        ...
        playerSnapshot.update(getPlayer(teamId, data, player, false))
            .then(() => {
              functions.logger.info(`Player ID ${playerId} updated.`);
            }).catch((error) => {
              functions.logger.error(`Update for Player ID ${playerId} failed.`,
                  error);
            });
      }
    });
